defmodule EcampusWeb.CoreComponents do
  @moduledoc """
  ...
  def back(assigns) do
    ~H"""
    <div class="mt-16">
      <.link navigate={@navigate} class="link link-hover">
  ...
  def show_modal(js \\ %JS{}, id) when is_binary(id) do
    js
    |> JS.dispatch("dialog-show", to: "##{id}")
  end

  def hide_modal(js \\ %JS{}, id) do
    js
    |> JS.dispatch("dialog-hide", to: "##{id}")
  end
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(EcampusWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(EcampusWeb.Gettext, "errors", msg, opts)
    end
  end
